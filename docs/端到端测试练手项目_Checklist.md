...
> 若只有 2 天总时间，可整节跳过，不影响「有计划+用例+缺陷+API」的完整交付。  
...
| Phase 4 | README + 报告 | 2026-07-14 | [x] |
...
2. 本项目为**测试流程练手项目**，不虚构商业背景。
3. 不必追求功能全覆盖；**流程完整 + 文档规范** 比「自动化很多条」更重要。
4. 交付时能讲清测试计划、用例、Bug 生命周期与 API 断言即可。
